---
name: evaluator
description: Use after implement-feature to score the current implementation against the active feature's spec.md Acceptance Criteria, emitting a pass/fail list. Read-only — makes no code changes.
---
...
Output: a pass/fail list reported inline to the user (one line per `Acceptance Criteria` scenario) — no new file is mandated, no existing file is edited.

Steps:
1. Resolve the active feature from `.specify/state` (fallback: newest dir under `specs/`).
2. Read `specs/<active-feature>/spec.md`, focusing on `Acceptance Criteria` (the Given/When/Then scenarios).
