...
- [ ] Task 1: module a (AC-1) [scope: src/a/**, tests/a/**]
- [ ] Task 2: module b (AC-2) [scope: src/b/**, tests/b/**]
- [ ] Task 3: touches a/x (AC-3) [scope: src/a/x.ts]
- [ ] Task 4: cross-cutting, no scope
EOF
node "$V" partition "$S1/tasks.md" > "$S1/out.txt" 2>&1
assert_eq "0" "$?" "partition exits 0 on valid tasks.md"
assert_contains "$S1/out.txt" "parallel ×2" "disjoint Task 1 + Task 2 batch in parallel"
assert_contains "$S1/out.txt" "Batch 1 (parallel ×2): Task 1, Task 2" "batch 1 groups the two disjoint tasks"
assert_contains "$S1/out.txt" "Task 4: (no scope)" "no-scope task reported solo"
rm -rf "$S1"
...
assert_contains "$S2/out.txt" "No batch >1" "overlapping scopes yield no parallel batch"
rm -rf "$S2"
