...
function readSkills(dir) {
  ...
  for (const name of readdirSync(base)) {
    ...
    // Walk sub-directories too: progressive-disclosure skills keep companions in
    // `references/`, and a view that dropped them would ship a SKILL.md linking files
    // that were never emitted. `filename` stays skill-dir-relative with forward slashes.
    const companions = [];
    const walk = (abs, rel) => {
      for (const f of readdirSync(abs)) {
        ...
        else companions.push({ filename: relPath, raw: readFileSync(full, "utf8") });
      }
    };
    walk(skillDir, "");
